...
public class RestExecuteAnomalyDetectionTaskAction extends BaseRestHandler {
    ...
    public RestExecuteAnomalyDetectionTaskAction(Client client,
                    ...
                    if (rawPath.endsWith(START_JOB)) {
                        try (ThreadContext.StoredContext context = threadPool.getThreadContext().stashContext()) {
                            ...
                            if (!anomalyDetectionIndices.doesAnomalyDetectionTaskExecutionIndexExist()) {
                                anomalyDetectionIndices.initAnomalyDetectionTaskExecutionIndexDirectly(
                                        ActionListener.wrap(
                                                res -> onCreateMappingsResponse(res, channel, task),
                                                e -> {
                                                    try {
                        ...
                        client.execute(
                                ...
                                ActionListener.wrap(res -> {
                                    ...
                                    List<TaskInfo> infos = tasks.stream()
                                            .filter(taskInfo -> StringUtils.equals(taskInfo.getHeaders().get(Task.X_OPAQUE_ID), TASK_ID_HEADER + ":" + taskId)
                                    ...
                                    log.error("Fail to list tasks failed.", exception);
                                })
                        );
                    }
                } catch (Exception e) {
    ...
    private void startTaskExecution(AnomalyDetectionTaskExecution task, String taskExecutionId, RestChannel channel) throws IOException {
        Runnable runnable = () -> {
            runAnomalyDetectionTask(task, taskExecutionId);
        };
    ...
    protected void runAnomalyDetectionTask(
