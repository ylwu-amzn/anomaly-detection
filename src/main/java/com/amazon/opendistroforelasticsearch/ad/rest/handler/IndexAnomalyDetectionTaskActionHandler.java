...
public class IndexAnomalyDetectionTaskActionHandler extends AbstractActionHandler {
    ...
    private void createAnomalyDetectionTask() {
        ...
        try {
            ...
            client.search(searchRequest, ActionListener.wrap(response -> onSearchTaskResponse(response), exception -> onFailure(exception)));
        } catch (Exception e) {
            onFailure(e);
        }
    }
    ...
    private void onSearchTaskResponse(SearchResponse response) {
        ...
        if (response.getHits().getTotalHits().value >= maxAnomalyDetectionTasks) {
            ...
            client.get(request, ActionListener.wrap(getResponse -> onGetAnomalyDetectorResponse(getResponse), exception -> onFailure(exception)));
        }
    }
    ...
    private void updateAnomalyDetectionTask(NodeClient client, String taskId) {
        ...
        client.get(request, ActionListener.wrap(response -> onGetAnomalyDetectionTaskResponse(response), exception -> onFailure(exception)));
    }
        ...
        return new RestActionListener<IndexResponse>(channel) {
            ...
            public void processResponse(IndexResponse response) throws Exception {
                if (response.getShardInfo().getSuccessful() < 1) {
                    channel.sendResponse(new BytesRestResponse(response.status(),
                ...
                if (response.status() == RestStatus.CREATED) {
                    String location = String.format(Locale.ROOT, "%s/%s", AnomalyDetectorPlugin.AD_BASE_DETECTION_TASKS_URI, response.getId());
