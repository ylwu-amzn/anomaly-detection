...
public class ADBatchTaskRunner {
    ...
    public ADBatchTaskRunner(
        ...
        clusterService.getClusterSettings().addSettingsUpdateConsumer(MAX_BATCH_TASK_PIECE_SIZE, it -> pieceSize = it);
    ...
    private void getNodeStats(ADTask adTask, ActionListener<DiscoveryNode> listener) {
        ...
        adStatsRequest.addAll(ImmutableSet.of(StatNames.AD_EXECUTING_BATCH_TASK_COUNT.getName(), StatNames.JVM_HEAP_USAGE.getName()));
        ...
        client.execute(ADStatsNodesAction.INSTANCE, adStatsRequest, ActionListener.wrap(adStatsResponse -> {
            ...
            candidateNodeResponse.stream().forEach(c -> {
                logger
                    .info(
                        "Node AD stats: node id: {}, running_task: {}, jvm_heap_usage: {}",
                        c.getNode().getId(),
            ...
            if (candidateNodeResponse.size() > 0) {
                if (candidateNodeResponse.size() == 1) {
                    ...
                    Long taskCount = (Long) candidateNodeResponse
                        .get(0)
                        .getStatsMap()
                        .get(StatNames.AD_EXECUTING_BATCH_TASK_COUNT.getName());
                    Optional<ADStatsNodeResponse> first = candidateNodeResponse
                        .stream()
                        .filter(c -> taskCount.equals(c.getStatsMap().get(StatNames.AD_EXECUTING_BATCH_TASK_COUNT.getName())))
                        .sorted(
                            (ADStatsNodeResponse r1, ADStatsNodeResponse r2) -> ((Long) r1
                                .getStatsMap()
                                .get(StatNames.JVM_HEAP_USAGE.getName()))
                                    .compareTo((Long) r2.getStatsMap().get(StatNames.JVM_HEAP_USAGE.getName()))
                        )
                        .findFirst();
    ...
    private ActionListener<ADBatchAnomalyResultResponse> wrappedListener(
        ADTask adTask/*, ActionListener<ADBatchAnomalyResultResponse> actionListener*/
    ) {
        // wrap original listener to process before return response/failure: 1.clean cache 2.track task stats
        String taskId = adTask.getTaskId();
    ...
    private void runFirstPiece(ADTask adTask, Instant executeStartTime, ActionListener<ADBatchAnomalyResultResponse> listener) {
        try {
            adTaskManager
                .updateADTask(
                    adTask.getTaskId(),
