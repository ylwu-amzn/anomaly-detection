...
public class ADTaskManager {
    ...
    private void onIndexADTaskResponse(
        ...
        client
            .execute(
                ...
                ActionListener.wrap(r -> { logger.info(r.getMessage()); }, exception -> { handleADTaskException(adTask, exception); })
            );
        if (function != null) {
            function.execute();
        }
    ...
    public void handleADTaskException(ADTask adTask, Exception exception) {
        ...
        if (exception instanceof ReceiveTimeoutTransportException) {
            ...
            String error = (exception instanceof IllegalArgumentException || exception instanceof AnomalyDetectionException)
