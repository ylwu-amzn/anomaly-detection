...
public class ADBatchTaskRemoteExecutionTransportAction extends HandledTransportAction<ADBatchAnomalyResultRequest, ADBatchAnomalyResultResponse> {
    ...
    @Inject
    public ADBatchTaskRemoteExecutionTransportAction(ActionFilters actionFilters, TransportService transportService,
    ...
    protected void doExecute(Task task, ADBatchAnomalyResultRequest request, ActionListener<ADBatchAnomalyResultResponse> listener) {
        adBatchTaskRunner.startADBatchTask(request.getTask(), task, listener);
    }
}
