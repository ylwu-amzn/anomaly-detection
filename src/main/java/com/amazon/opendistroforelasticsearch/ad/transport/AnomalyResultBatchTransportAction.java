...
//    private final TransportService transportService;
    ...
    private final AnomalyResultHandler anomalyResultHandler;
...
//                    () -> stateManager.getAnomalyDetector(detectorId, onGetDetector(listener, detectorId, taskId, request)));
    ...
    private void checkTaskCancelled(String taskExecutionId) {
        if (taskMap.containsKey(taskExecutionId) &&
                taskMap.get(taskExecutionId).isCancelled()) {
...
//                .orElse(0L);
            ...
            LOG.info("start first piece from {} to {}, interval {}, dataStartTime {}, dataEndTime {}", dataStartTime, timeStamp, interval, dataStartTime, dataEndTime);
            getFeatureData(listener, taskId, anomalyDetector, dataStartTime,
                    timeStamp, dataEndTime, interval, Instant.now());

        }, exception -> handleExecuteException(exception, listener, adID));
    }

    private void getFeatureData(ActionListener<AnomalyResultBatchResponse> listener,
                                String taskId,
                                AnomalyDetector detector,
                                long dataStartTime,
                                long timeStamp,
                                long dataEndTime,
                                long interval, Instant executeStartTime) {
        checkTaskCancelled(taskId);
        featureManager.getFeatures(detector, dataStartTime, timeStamp,
                onFeatureResponseLocalRCF(taskId, detector, listener,
                        timeStamp,
                        dataEndTime,
                        interval, executeStartTime));
    }
    ...
    private ActionListener<List<SinglePointFeatures>> onFeatureResponseLocalRCF(
        ...
        return ActionListener.wrap(featureList -> {
            ...
            getScoreFromRCF(detector, taskId, detector.getEnabledFeatureIds().size(), featureList,  timeStamp,
             dataEndTime,
             interval,
             executeStartTime, listener);
//            runNextPiece(detector, taskId, timeStamp, dataEndTime, interval, listener);
    ...
    private void getScoreFromRCF(AnomalyDetector detector, String taskId, int enabledFeatureSize,
                                 List<SinglePointFeatures> featureList, long timeStamp,
                                 long dataEndTime,
                                 long interval,
                                 Instant executeStartTime, ActionListener<AnomalyResultBatchResponse> listener) {
        ...
        for (int i = 0; i < featureList.size(); i++) {
            ...
            for (int j = 0; j < enabledFeatureIds.size(); j++) {
                featureData.add(new FeatureData(enabledFeatureIds.get(j), enabledFeatureNames.get(j),
        ...
        anomalyResultHandler.bulkIndexAnomalyResult(anomalyResults, ActionListener.wrap(response -> {
    ...
    private void runNextPiece(AnomalyDetector detector, String taskId, long timeStamp, long dataEndTime, long interval, ActionListener<AnomalyResultBatchResponse> listener) {
        if (timeStamp < dataEndTime) {
            long endTimeStamp = timeStamp + (PIECE_SIZE - SHINGLE_SIZE + 1) * interval > dataEndTime ?
                    dataEndTime : timeStamp + (PIECE_SIZE - SHINGLE_SIZE + 1) * interval;
            rateLimiter.acquire(60 / PIECES_PER_MINUTE);
            LOG.info("start next piece start from {} to {}, interval {}", timeStamp, endTimeStamp, interval);
            getFeatureData(listener, taskId, detector, timeStamp - (SHINGLE_SIZE - 1)* interval, endTimeStamp, dataEndTime, interval, Instant.now());
        } else {
